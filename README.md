...
| `build-node` | `node`           | A _Node_-compatible build that we can upload to NPM or include as a dependency during local development by running `make link`.                                                                           |
...
| `link`       |                  | Uses your Node Package Manager's `link` command (default `npm link`) so you can use the built Node package in your other local projects.                                                                  |
